...
{
    public partial class FormVerDictadosDocenteVerRendimiento : Form
    {
        public FormVerDictadosDocenteVerRendimiento(int pKey, Docente pDocente)
        {
            InitializeComponent();
            this.CenterToParent();
            ID = pKey;
        ...
        FormVerRendimientoDocente FormVerRendimientoDocente;

        private void FormVerDictadosDocenteVerRendimiento_Load(object sender, EventArgs e)
        {
        ...
        {
            dgvDictados.Rows.Clear();
            LD = BLL_Dictado.Ver_Dictados_del_Docente_Inactivos(IDDocente);
            MostrarDatosEnGrilla(dgvDictados, LD);
        }

        private void btnVerRendimientoDictado_Click(object sender, EventArgs e)
        {
            if (FormVerRendimientoDocente == null)
            {
                if (dgvDictados.Rows.Count>0)
                {
                    Dictado D = ObtenerDictado((int)dgvDictados.SelectedRows[0].Cells[0].Value);
                    FormVerRendimientoDocente = new FormVerRendimientoDocente(ID, Docente, D);
                    FormVerRendimientoDocente.FormClosed += FormVerRendimientoDocente_FormClosed;
                    FormVerRendimientoDocente.ShowDialog();
                }                
            }
        }

        private void FormVerRendimientoDocente_FormClosed(object sender, FormClosedEventArgs e)
        {
            FormVerRendimientoDocente = null;
        }
