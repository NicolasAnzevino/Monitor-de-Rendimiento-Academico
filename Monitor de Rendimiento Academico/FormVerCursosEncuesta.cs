...
{
    public partial class FormVerCursosEncuesta : Form
    {
        public FormVerCursosEncuesta(int pKey)
        {
            InitializeComponent();
            this.CenterToParent();
        ...
        BLL_Curso BLL_Curso;
        List<Curso> LC;
        FormCrearEncuesta FormCrearEncuesta;

        private void FormVerCursosEncuesta_Load(object sender, EventArgs e)
        {
        ...
        private void btnCrearEncuesta_Click(object sender, EventArgs e)
        {
            try
            {
                if (FormCrearEncuesta == null)
                {
                    if (dgvCursosActivos.Rows.Count>0)
                    {
                        Curso C = ObtenerCurso((int)dgvCursosActivos.SelectedRows[0].Cells[0].Value);
                        FormCrearEncuesta = new FormCrearEncuesta(ID, C);
                        FormCrearEncuesta.FormClosed += FormCrearEncuesta_FormClosed;
                        FormCrearEncuesta.ShowDialog();
                    }
                }
            }
            catch (Exception ex) { throw new Exception(ex.Message); }
        ...
        private void FormCrearEncuesta_FormClosed(object sender, FormClosedEventArgs e)
        {
            FormCrearEncuesta = null;
        }
