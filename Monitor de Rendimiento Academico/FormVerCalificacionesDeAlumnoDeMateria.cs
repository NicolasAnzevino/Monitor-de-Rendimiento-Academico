...
{
    public partial class FormVerCalificacionesDeAlumnoDeMateria : Form
    {
        public FormVerCalificacionesDeAlumnoDeMateria(int pKey, Cursada_de_Alumno pCursada, Materia pMateria)
        {
            InitializeComponent();
            this.CenterToScreen();
        ...
        FormModificarCalificacion FormModificarCalificacion;

        private void FormVerCalificacionesDeAlumnoDeMateria_Load(object sender, EventArgs e)
        {
        ...
        private void btnEliminarCalificacion_Click(object sender, EventArgs e)
        {
            try
            {
                if (LE.Count > 0)
                {
                    ...
                    if (MessageBox.Show("¿Está seguro de eliminar esta calificación?", "Solicitud de Confirmación", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        BLL_Cursada_de_Alumno.Eliminar_Evaluacion_de_Alumno(EA);
                        MessageBox.Show("La Calificación se eliminó correctamente", "Atención", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        LogManager.Escribir(ID, "Eliminó la Calificación " + EA.Codigo, 3);
        ...
        private void btnModificarCalificacion_Click(object sender, EventArgs e)
        {
            try
            {
                if (FormModificarCalificacion == null)
                {
                    if (LE.Count > 0)
                    {
                        Evaluacion_de_Alumno EA = Obtener_Evaluacion_de_Alumno(int.Parse(dgvEvaluacionesCalificacion.SelectedRows[0].Cells[0].Value.ToString()));

                        FormModificarCalificacion = new FormModificarCalificacion(ID, Cursada.Alumno, Cursada.Curso, Materia, EA.Evaluacion, EA);
                        FormModificarCalificacion.FormClosed += FormModificarCalificacion_FormClosed;
                        FormModificarCalificacion.ShowDialog();
        ...
        private void FormModificarCalificacion_FormClosed(object sender, FormClosedEventArgs e)
        {
            FormModificarCalificacion = null;
        }
    }
}
