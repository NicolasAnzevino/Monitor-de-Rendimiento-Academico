...
{
    public partial class FormVerCursosVerRendCurso : Form
    {
        public FormVerCursosVerRendCurso(int pKey)
        {
            InitializeComponent();
            this.CenterToParent();
        ...
        BLL_Curso BLL_Curso;
        List<Curso> LC;
        FormVerRendimientoCurso FormVerRendimientoCurso;

        private void FormVerCursosVerRendCurso_Load(object sender, EventArgs e)
        {
        ...
        private void btnVerRendimientoCurso_Click(object sender, EventArgs e)
        {
            try
            {
                if (dgvCursosActivos.Rows.Count > 0)
                {
                    if (FormVerRendimientoCurso == null)
                    {
                        Curso C = ObtenerCurso((int)dgvCursosActivos.SelectedRows[0].Cells[0].Value);
                        FormVerRendimientoCurso = new FormVerRendimientoCurso(ID, C);
                        FormVerRendimientoCurso.FormClosed += FormVerRendimientoCurso_FormClosed;
                        FormVerRendimientoCurso.ShowDialog();
                    }
                }
        ...
        private void FormVerRendimientoCurso_FormClosed(object sender, FormClosedEventArgs e)
        {
            FormVerRendimientoCurso = null;
        }
    }
}
